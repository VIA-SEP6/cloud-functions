...
module.exports = {
	get: functions.region("europe-west1").https.onCall(async (data, context) => {
		...
		return getMovieRequest(movieId, append_to_response).catch((err) => {
	...
	getPopular: functions
		...
		.https.onCall(async (data, context) => {
			return getMoviesRequest("popular").catch((err) => {
				new HttpsError("failed-precondition", err);
			});
		}),
	getUpcoming: functions
		.region("europe-west1")
		.https.onCall(async (data, context) => {
			return getMoviesRequest("upcoming").catch((err) => {
				new HttpsError("failed-precondition", err);
			});
		}),
	getTopRated: functions
		.region("europe-west1")
		.https.onCall(async (data, context) => {
			return getTopRatedMoviesRequest().catch((err) => {
				new HttpsError("failed-precondition", err);
			});
		}),
	search: functions
		...
		.https.onCall(async (data, context) => {
			...
			return searchMovieGetRequest(query).catch((err) => {
				new HttpsError("failed-precondition", err);
			});
		})
};
